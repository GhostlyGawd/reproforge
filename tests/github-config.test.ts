...
const privateKeyObject = generateKeyPairSync("rsa", { modulusLength: 2048 })
  .privateKey;
const pkcs8PrivateKey = privateKeyObject
  .export({ format: "pem", type: "pkcs8" })
  .toString();
const pkcs1PrivateKey = privateKeyObject
...
const environment = {
  ...
  GITHUB_APP_ID: "12345",
  GITHUB_APP_PRIVATE_KEY: pkcs8PrivateKey,
...
describe("GitHub App configuration", () => {
  ...
  it("accepts the PKCS#1 RSA private-key format generated by GitHub Apps", () => {
    expect(
      parseGitHubConfig({
        ...environment,
        GITHUB_APP_PRIVATE_KEY: pkcs1PrivateKey,
      }).credentials.privateKey,
    ).toBe(pkcs1PrivateKey);
  });
  ...
  it.each([
    ...
    ["non-RSA key", { ...environment, GITHUB_APP_PRIVATE_KEY: ecPrivateKey }],
